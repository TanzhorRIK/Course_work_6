...
class Home(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        active = MailingSettings.objects.filter(
            mailing_status='AC').values_list().count(),
...
class MailingCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        MailFormset = inlineformset_factory(MailingSettings, Mail,
                                            form=MailForm, can_delete=False,
                                            extra=1)
        if self.request.method == 'POST':
            context_data['formset'] = MailFormset(self.request.POST or None,
    ...
    def form_valid(self, form):
        ...
        for fo in formset:
            if fo.is_valid():
                ...
                if all_clients:
                    client_email = list(
                        MailingClient.objects.all().values_list('contact_email',
                                                                flat=True))
        if form.is_valid():
        ...
        try:
            if self.object.mailing_time_start.timestamp() <= ct.timestamp() <= self.object.mailing_time_end.timestamp():
                sending = send_mail(mailing_subject, mailing_body,
            ...
            MailingTry.objects.create(mailing=self.object,
                                      mailing_try=datetime.now(),
